...
- tabular data transformations in R 

- communicating and interpreting grouped summaries

**IMPORTANT:** We've set `eval=FALSE` in each of the R code chunks below so that we can render the homework without filling in valid R solutions.  As you fill in answers and progress through the assignment, make sure to set `eval=TRUE` in the code chunks you complete.

# Data import and assessment

The cell below imports select columns from the 2019 dataset as an R `tibble`. The file is big, so this may take a while, and then have a quick look at the first few rows and columns.
...
Check the number of records (interviews conducted) reported and variables measured for 2019 by reviewing the [data documentation](https://www.cdc.gov/brfss/annual_data/annual_2019.html), and then answer the following questions in a few sentences:
...
Skim the [overview documentation](https://www.cdc.gov/brfss/annual_data/2019/pdf/overview-2019-508.pdf) for the 2019 BRFSS data. Focus specifically on the 'Background' and 'Data Collection' sections, read selectively for relevant details, and answer the following questions in a few sentences:
...
v. What is the sampling frame (*i.e.*, all individuals who could possibly be sampled)?
...
```
the lecture slides provide additional examples.  What selecting function (`starts_with`, `contains()`...) can you use to get the ACE variables?
...
For the final dataset, drop the respondent answers to individual questions, the missingness indicator, and select just the derived indicator variables along with general health, sex, age, and smoking status. Store the result as `final_brfss_data`.
...
Now that you have a clean dataset, you'll use grouping and aggregation to compute several summary statistics that will help you explore whether there is an association between experiencing adverse childhood conditions and self-reported health, smoking status, and depressive disorders in areas where the ACE module was administered.
...
*Is there an observed association between having ACEs and general health, smoking status, and depression among survey respondents who answered the ACE questions?*
...
### Question 19: Scope of inference

The BRFSS carefully collects a representative sample of U.S. adults with phone access. Do your findings suggest an association in the general public, and what concerns might limit generalization? Answer in two sentences.
...
Notice that the language 'association' is non-causal: we don't say that ACEs cause (or don't cause) poorer health outcomes. This is intentional, because the BRFSS data are what are known as 'observational' data, i.e. not originating from a controlled experiment. There could be unobserved factors that explain the association. In a few sentences, propose a few potential factors not included in this homework that could explain this association between adverse childhood experiences and health.
