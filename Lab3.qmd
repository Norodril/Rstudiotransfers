...
# Lab 3: Sampling designs and statistical bias overview with R
...
#unbiased- every seed in the pop has an equal probability of being selected
set.seed(40221)
...
```{r}
# Calculate the bias
#mean(sampe_means) = the average of all 1000 simulated sample means
...
# Create histogram for the biased sample
hist_samp2 <- ggplot(sample2, aes(x = diameter)) +
  geom_histogram(binwidth = 0.3, fill = "orange", alpha = 0.8, boundary = 0) +
  geom_vline(aes(xintercept = mean_sample2_diameter), color = "red", linetype = "dashed", size = 1) +
  labs(
    x = "Diameter (mm)",
    y = "Number of seeds in biased sample",
    title = "Biased Sample Histogram"
  ) +
  xlim(0, 6) +
  theme_minimal()
...
First try drawing a random sample from the population with seed 40821:
...
```{r}
set.seed(40821)
#randomly selects 200 rows (hawks) from the total population
sample_hawks <- population_hawks[sample(1:nrow(population_hawks), 200), ]
```
:::
...
::: callout
```{r}
sample_hawks_mean <- mean(sample_hawks$length)

print(sample_hawks_mean)
```
:::
...
set.seed(40822)
sample_hawks_biased <- population_hawks %>%
  sample_n(size = 200, weight = weight)

sample_hawks_biased_mean <- mean(sample_hawks_biased$length)

print(sample_hawks_biased_mean)
```
:::
...
pop_mean_hawks <- mean(population_hawks$length) # True population mean

for (i in 1:nsim) {
  samp <- population_hawks %>%
    sample_n(size = 200, weight = weight)
  
  samp_means_hawks[i] <- mean(samp$length)
}

avg_diff_hawks <- mean(samp_means_hawks - pop_mean_hawks)
...
Reflect a moment on your simulation result in question 10. If instead *female* mortality is higher and specimens for measurement are collected opportunistically, as described in the previous sampling design, do you expect that the average length in the sample will be an underestimate or an overestimate of the population mean? Explain why in 1-2 sentences, and carry out a simulation to check your intuition.
...
```{r}
'''
If female morality is higher (females are more likely to die) and more females are sampled, the sample will contain disproportionately large individuals (females are bigger on average). This will cause the sample mean to overestimate the true population mean, since the larger females will pull the average up compared to the overall population mean.
'''
```
:::
...
::: callout
```{r}
#this redraws the same biased sample as q9
set.seed(40822)
sample_hawks_biased <- population_hawks %>%
  sample_n(size = 200, weight = weight)

#computes the biased sample mean
sample_mean <- mean(sample_hawks_biased$length)

#computes the bias adjustment factor
#inverse of weight
sample_hawks_biased$bias_adjustment <- (1/sample_hawks_biased$weight) / sum(1 / sample_hawks_biased$weight)

#computes the corrected/weighted sample mean
sample_mean_corrected <- sum(sample_hawks_biased$length * sample_hawks_biased$bias_adjustment)

print(sample_mean)
print(sample_mean_corrected)
```
:::
